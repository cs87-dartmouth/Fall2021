...
Triangle::Triangle(const json &j) : m_face_idx(0)
{
    ...
    mesh->vs        = {j["positions"][0], j["positions"][1], j["positions"][2]};
    mesh->Fv        = {{0, 1, 2}};
    mesh->Fm        = {0};
    ...
    if (j.contains("normals") && j.at("normals").is_array())
    {
        ...
        {
            mesh->ns = {j["normals"][0], j["normals"][1], j["normals"][2]};
            mesh->Fn = mesh->Fv;
        }
        else
                               ...
                               HitInfo &hit, const Material *material)
{
    ...
    // TODO: Fill in the geometric normal with the geometric normal of the triangle (i.e. normalized cross product of
    // the sides)
    Vec3f gn = Vec3f(0.0f);
